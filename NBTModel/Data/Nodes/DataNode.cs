...
namespace NBTExplorer.Model
{
    ...
    public class DataNode
    {
        ...
        public DataNode? LoadChild(string childName) 
        {
            var child = this.Nodes.FirstOrDefault(n => n.NodeName == childName);
            child?.Load();
            return child;
        }
        public object? GetChildValue(string childName)
        {
            _ = new TagNodeString().Data;
            _ = new TagNodeInt().Data;
            var child = this.Nodes.FirstOrDefault(n => n.NodeName == childName);
            if (child is TagDataNode tag) 
            {
                dynamic tagValue = tag.Tag;
                return tagValue.Data;
            }
            return null;
        }
        public T GetChildValue<T>(string childName)
        {
            return (T)GetChildValue(childName, typeof(T));
        }
        public object GetChildValue(string childName, Type type)
        {
            var value = GetChildValue(childName);
            if (value == null) return null;
            return Convert.ChangeType(value, type);
        }
        public void LoadAllChilds()
        {
            foreach (TagCompoundDataNode node in this.Nodes)
            {
                node.Load();
                node.LoadAllChilds();
            }
